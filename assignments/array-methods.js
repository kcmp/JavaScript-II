...
// let fullname = runners.forEach((element) => fullName.push(`${element.first_name} ${element.last_name}`);
...
let ticketPriceTotal = runners.reduce(function(arr, currentValue) {
    return {donation: arr.donation + currentValue.donation};
});
...
// Problem 1
//Send a thank you message for donation
runners.forEach(function(element){
    console.log(`Thank you ${element.first_name} ${element.last_name} from ${element.company_name} for you gift of \$${element.donation}.`)
});

// Problem 2
//A major company wants to match the gifts made at a 50% rate
let matchingGifts = runners.map(money => money.donation * .5).reduce((arr, currentValue) => arr + currentValue);
console.log(matchingGifts); 


// Problem 3
//Create a list of runners whose last name begins with P
// Create a list of donors who gave $100 or more
let bigSpenders = [];
bigSpenders = runners.filter(function (item) {
  return item.donation >= 100;}).map(function(item) {
  return `${item.first_name} ${item.last_name} : ${item.email}`
})
console.log(bigSpenders);

// let bigSpenders = runners.filter(item())